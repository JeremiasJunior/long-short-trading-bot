...
class fourteenbis_live():
        ...
        while(canTrade):
            ...
            if(len(self.godhandlog['price']['time']) >= period):
                ...
                if (position_open == True and count >= min_count):
                    ...
                    if(self.godhandlog['positions'][curr_positionkey]['type'] == 'short_spread'):
                        '''
                        dps preciso ver se o ultimo parametros do if realmente importa
                        '''    

            
                        if(self.stopvalue(self.godhandlog['positions'][curr_positionkey]['x_price'][0], 
                                          self.godhandlog['positions'][curr_positionkey]['y_price'][0],
                                          current_x_price,
                                          current_y_price,
                                          self.godhandlog['positions'][curr_positionkey]['x_volume'],
                                          self.godhandlog['positions'][curr_positionkey]['y_volume'],
                                          self.godhandlog['positions'][curr_positionkey]['type'], curr_expo) == True):
                        ...
                        #if((curr_return >= tp_return) or (spread_today <= tp_short) or curr_min >= 1005):
                        if((curr_return >= tp_return) or (spread_today <= tp_short)) or force_close == True:   
                            #fecha posição shortspread
                            _a, _b, _c, _d =  self.close_position(curr_positionkey, server=connected_server)
                            returns = _d
                            position_open=False
                            position_key = ''
                            ...
                            self.godhandlog['Long'].append(curr_long)
                            self.godhandlog['Short'].append(curr_short)
                    ...
                    elif(self.godhandlog['positions'][curr_positionkey]['type'] == 'long_spread'):
                        print('longspread------')
                        #if((curr_return >= tp_return) or (spread_today >= tp_long) or curr_min >= 1005):
                        ...
                        if((curr_return >= tp_return) or (spread_today >= tp_long)) or force_close == True:   
                            #fecha posição longspread
                            _a, _b, _c, _d = self.close_position(curr_positionkey, server=connected_server)
    ...
    def short_spread(self, y_symbol, x_symbol, position_size, ratio, server):
            ...
            print({'key':'{},{}'.format(str(x_ticket), str(y_ticket)), 
                'x_price':x_price,
                'y_price':y_price,
                'type':'long_spread',
                'x_volume':x_volume,
    ...
    def close_position(self, position_ticket, server):

        x_ticket, y_ticket = position_ticket.split(',')

        x_close_data = mt5_close(x_ticket, server).copy()
        y_close_data = mt5_close(y_ticket, server).copy()
        ...
        self.godhandlog['positions'][position_ticket]['time'][1] = str(datetime.now())


        if self.godhandlog['positions'][position_ticket]['type'] == 'long_spread':
            
            self.godhandlog['positions'][position_ticket]['y_price'][1] = y_close_data[4]
            self.godhandlog['positions'][position_ticket]['x_price'][1] = x_close_data[3]
            ...
            self.godhandlog['profit'].append(_lucro)



        if self.godhandlog['positions'][position_ticket]['type'] == 'short_spread':
            
            self.godhandlog['positions'][position_ticket]['y_price'][1] = y_close_data[3]
            self.godhandlog['positions'][position_ticket]['x_price'][1] = x_close_data[4]
