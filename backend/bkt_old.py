...
class bkt_godhand():
    ...
    def __new__(self,  dependent_ticker, df_x,
        ...
        for df_iteration in range(len(df_index)): # da pra melhorar isso aqui
            ...
            if(current_x_price == 0):
                current_x_price = self.tools.bkt_getprice(df_x, column, df_iteration-1)[0]
            if(current_y_price == 0):
                current_y_price = self.tools.bkt_getprice(df_y, column, df_iteration-1)[0]
    ...
    def roundlot2(x_price, y_price, lot):
    ...
    def stopvalue(df_xprice, df_yprice, curr_xprice, curr_yprice, x_volume, y_volume, type, expo,tp=0.07, sl=-0.03):
